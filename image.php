<?php
    ...
    if (strpos($_SERVER['PATH_INFO'], 'epc.php') !== false || is_file($_SERVER['DOCUMENT_ROOT'] . $prefix_folder . '/images' . $_SERVER['PATH_INFO'])) {

        $images = array_values(array_filter(image_list(dirname($_SERVER['DOCUMENT_ROOT'] . $prefix_folder . '/images/' . $_SERVER['PATH_INFO']) . '/'), 'thumbnail_filter'));
        ...
        echo "\n\t\t\t<div id=\"img\">\n\t\t\t\t<h2>$title</h2>\n\t\t\t\t<img src=\"" . $prefix . '/images' . $_SERVER['PATH_INFO'] . "\" alt=\"house image\" />\n\t\t\t</div>\n";
